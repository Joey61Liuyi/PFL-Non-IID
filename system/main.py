...
def run(goal, dataset, num_labels, device, algorithm, model, local_batch_size, local_learning_rate, global_rounds, local_steps, join_clients, 
        ...
        time_select, time_threthold, M, mu, itk, alphaK, sigma, xi, genotype, run_name, resume_path, choose_client, step_alignment, start_epoch=0, iid=0.5):
    ...
    for i in range(times):
        ...
        if algorithm == "FedAvg":
            ...
            server = FedMD(device, dataset, algorithm, Model, local_batch_size, local_learning_rate, global_rounds,
                            local_steps, join_clients, num_clients, i, eval_gap, client_drop_rate, train_slow_rate,
                            send_slow_rate, time_select, goal, time_threthold, run_name, choose_client, step_alignment, iid)
...
if __name__ == "__main__":
    ...
    for iid in [0.01,0.1, 0.5, 1,10]:
        ...
        # FedMD
        parser.add_argument('-stepalignment', "--sa", type=int, default=1)
        ...
        if config.model in Networks:
            ...
            wandb_project = "PAS+X"
            run_name = "{}-{}-{}-{}".format(config.model, algorithm, config.dataset, K)
